...
const nutritionRows = nutritionSeed as NutritionRow[];
const ingredientRows = ingredientsSeed as Ingredient[];
const monthlySeason = (shenzhenSeason as { monthly: Record<string, string[]> }).monthly;
...
function weeklySeasonalNames(date: Date): string[] {
  ...
  return uniqueNames([
    ...current.slice(start, start + 4),
    ...next.slice(0, Math.max(0, 6 - (current.length - start))),
  ]);
}
